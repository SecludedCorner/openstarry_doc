...
| 問題 | 影響 |
|------|------|
| IListener 同時處理輸入和輸出 | 違反單一職責，混淆色蘊·輸入 (Listener) 與色蘊·輸出 (UI) |
...
├── IListener (色蘊·輸入) — 輸入接收，推送 InputEvent 到隊列
...
```typescript
/**
 * Listener interface — receives external input (色蘊·輸入).
 */
...
> Listener 插件 (色蘊·輸入) 專注於接收外部輸入。兩者同屬色蘊，各司其職。
