...
在 OpenStarry 中，「受蘊」是一個橫跨系統的情感性評價機制，包含三種感受：
*   **苦受 (Dukkha)：** 失敗、超時、資源耗盡——觸發重試、降級或告警。
*   **樂受 (Sukha)：** 成功、達標、正向反饋——強化當前策略。
*   **捨受 (Upekkha)：** 常規運作——維持平衡，不觸發特殊反應。

以「苦受」為例的運作流程：
*   **苦受感應 (Core):** `SafetyMonitor` 捕獲工具執行失敗，產生苦受信號。
*   **苦受詮釋 (Guide):** `VedanaAware_Guide` 將失敗轉化為帶有強烈負面暗示的 Prompt。
*   **苦受反應 (LLM):** Agent 感覺到「痛」（任務受阻），觸發自我反思與策略調整。
