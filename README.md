...
> 門面級聲明：本庫所有「生命／識／物種」語彙皆為**功能性設計隱喻**（二諦式邊界宣告詳見 Deep Dive 14 與 doc 47/52）——我們宣稱的是可測試的行為與架構性質，不宣稱現象意識、感受質或生物學意義的生命；佛學名相為架構本體論的工程映射，不宣稱教義等同。哪些已被代碼證明、哪些仍是願景，[兌現帳本](./TENETS_FULFILLMENT.md)逐條分判。

---

## 🧭 導讀 (Start Here)

**這個文件庫是一個 Agent OS 參考架構的時間膠囊**：十大宣言是「AI 落地那一天」的預先答案，姊妹 repo 的五萬行代碼與 3179 個通過測試是可建造性的證明，而過程的失敗數據（87.5% spec-vs-merged gap、96% closure 灌水——自家稽核實測）原樣公開。
...
> （本節借佛教語彙作生命週期意象，非嚴格教義對應：「緣起」pratītyasamutpāda 與「寂滅」nirvāṇa 於教典各有專義，此處僅取因待而生、滅盡而息的工程意象。嚴格的名相映射見 [Deep Dive 14](./Agent_Core_Components_Deep_Dive/14_Agent_Core_Philosophy_Five_Aggregates.md)。）
...
OpenStarry 的強大在於其聲明式的配置。以下是一個具備「痛覺」與「檔案操作能力」的最小 Agent 定義（四要素齊備：context manager＋輸入色身＋工具＋provider——缺 context manager 插件 core 會直接 throw）：

```jsonc
// agent.json（最小可跑形狀；完整實例見代碼 repo 的 configs/basic-agent.json 與 GETTING_STARTED.md）
{
  "identity": { "id": "dev-bot-01", "name": "Resilient Developer" },
  "cognition": { "provider": "gemini", "model": "gemini-2.0-flash" },
  "capabilities": { "tools": ["fs.read", "fs.list"], "allowedPaths": ["."] },
  "plugins": [
    // [記憶] 上下文管理：強制插件類別（Tenet #9）
...
> 📒 各宣言的誠實兌現狀態（已證明／已證至 N=2／明確不宣稱）見 **[十大宣言兌現帳本 (TENETS_FULFILLMENT.md)](./TENETS_FULFILLMENT.md)**（v0.59.4-alpha，每項主張附證據指標）。
>
...
*   **識 Identity (Guide)**：**我執框架（Ego Framework）**——行為約束與自我收斂機制。Guide 定義了 Agent 的身份認同、行為邊界與約束規則，使 Agent 在發散的可能性中收斂到預期行為。沒有 Guide，Core 只是無方向的運算力。`[Cycle 02-2 A-1 修正註記：教義上我執不是收斂約束本身，而是煩惱的根源；收斂是「我執→煩惱→行動動力→約束」因果鏈的下游產物（見 Deep Dive 14 §5）。本條工程描述取因果鏈的末端效果。]`
...
對應代碼版本：**v0.59.4-alpha** ｜ 測試：**300 files / 3179 passed / 0 failed / 4 skipped** ｜ 微內核純度檢查：PASS（`scripts/check-purity.sh` 機器強制）。
