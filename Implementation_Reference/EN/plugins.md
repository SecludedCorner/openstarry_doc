...
> ⚠️ **[漂移更正 — v0.59.6] 套件數已修正：47 package directories = 46 loadable plugins + 1 shared library (`mcp-common`)。** 原文「43 plugin directories as of v0.58.0-alpha」已陳舊。實測依據：`openstarry_plugin/` 下 `package.json` 共 47 個（`find openstarry_plugin -maxdepth 2 -name package.json -not -path "*/node_modules/*"` = 45；pnpm workspace glob `../openstarry_plugin/*` 見 `openstarry/pnpm-workspace.yaml:4` 全數納管）。其中 `mcp-common`（`@openstarry-plugin/mcp-common` v0.4.0，`mcp-common/package.json:1-22`）無 `test` script、不依賴 `@openstarry/sdk`、僅匯出協定 types/constants ＝ 共享函式庫，非可載入 plugin（對照可載入者如 `mcp-server/package.json:20-23` 依賴 `@openstarry/sdk` 且有 `test`）。下方表格為策展精選（非窮舉），其數字與標籤仍正確（`mcp-common` 已標 shared library）。

OpenStarry's plugin workspace contains **47 package directories** as of v0.59.x-alpha — **46 loadable plugins + 1 shared library (`mcp-common`)** (count corrected 2026-06-16 against shipped code; the tables below are a curated selection, not exhaustive — see `openstarry_plugin/` for the full set).
