...
**檔案**: `agent_dev/openstarry/packages/sdk/src/types/distributed-alaya.ts`（`interface IBijaStore`，約 :118）
...
**檔案**: `agent_dev/openstarry/packages/sdk/src/types/distributed-alaya.ts`（`interface ISeedSignatureService`，約 :148）
...
**檔案**: `agent_dev/openstarry/packages/sdk/src/types/distributed-alaya.ts`（`type VectorClock`，約 :103）
...
> ⚠️ **[誠實範圍 — v0.59.4-alpha drift 稽核 2026-06-16]** 本節（與 §7.1 的「自動啟動 self-activating／無條件 COMPLIANT」表述）是**設計願景，非已落地的能力**。實作真相以 `openstarry_plugin/distributed-alaya/src/remote-peer.ts:19-23` 的 honest-scope 註記為準：**單機傳播（named pipe／UDS）、信任父進程的 key 分發；已證至 N=2；N>2 gossip 與跨主機 transport 明確未實作（OUT OF SCOPE）**。插件**非自動啟動**——須由 host config 顯式載入 `distributed-alaya` 並提供 `peers`，且未列於任何 default config（`agent_dev/openstarry/configs/*` 0 引用）。宣言 #6 之兌現帳本（`TENETS_FULFILLMENT.md`）明確標界於「N=2，群體規模未證」。下列 N-Agent 共識/容錯敘述請讀作目標態。
