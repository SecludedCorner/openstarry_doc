...
**OpenStarry** 是一个重新定义智能代理 (AI Agent) 构建方式的核心架构。它参考了现代操作系统的设计哲学，融合东方「五蕴」思想，旨在打造一个高度模块化、安全、且具备拟人化生命特征的代理协调层。
...
插件决定了 Agent 的能力边界。包括通信协议 (Protocol)、自我反思 (Reflection) 与状态记忆 (Memory) 插件。这让同一个 Core 可以随时从「代码专家」转化为「设备监控员」。
...
1. **缘起 (Origination)**：环境层检测到需求。
2. **调度 (Scheduling)**：管理层根据需求匹配所需的插件。
3. **生起 (Arising)**：容器层载入核心并动态注入能力。
...
### 1. 代理即操作系统进程 (Agent as OS Process)
Agent 不是一次性的脚本，而是具备持久生命周期、可被守护进程 (Daemon) 管理、监控、重启的数字实体。它有自己的 PID，有自己的状态，就像一个活着的进程。

### 2. 一切皆插件 (Everything is a Plugin)
系统的每一个器官都是可替换的。工具是插件，监听器是插件，LLM 大脑是插件，甚至记忆策略和通信协议也是插件。Core 只是一个空的插座板，所有能力都来自外部挂载。
...
*   **物理隔离:** 编译后的 Core 二进制文件**严禁包含任何插件代码**。
...
不仅是执行循环，更是控制回路。系统将用户目标视为参考输入，将 Context 视为状态反馈，将 Tool Call 视为控制变量。Agent 的本质是一个不断最小化「目标与现状误差」的智能控制器。
...
* [07_支持引擎生态系统 (Supporting Engines Ecosystem)](./Architecture_Documentation/07_Supporting_Engines_Ecosystem.md)
* [08_命令与工具设计 (Command & Tool Design)](./Architecture_Documentation/08_Command_And_Tool_Design.md)
* [09_通信协议策略 (Communication Protocol Strategy)](./Architecture_Documentation/09_Communication_Protocol_Strategy.md)
...
* [22_代理协调层：归一化与适配 (Agent Coordination Layer: Normalization)](./Architecture_Documentation/22_Agent_Coordination_Layer_Normalization.md)
...
* [01_执行循环 (Execution Loop)](./Agent_Core_Components_Deep_Dive/01_Execution_Loop.md)
* [02_通信接口 (Communication Interface)](./Agent_Core_Components_Deep_Dive/02_Communication_Interface.md)
...
* [05_插件基础设施集成 (Plugin Infrastructure Integration)](./Agent_Core_Components_Deep_Dive/05_Plugin_Infrastructure_Integration.md)
...
* [02_MCP 协议集成 (MCP Protocol Integration)](./Plugin_System_Architecture/02_MCP_Protocol_Integration.md)
...
* [05_高级 UI 与设备示例 (Advanced UI & Device Example)](./Plugin_System_Architecture/05_Advanced_UI_And_Device_Example.md)
* [06_数据验证示例 (Data Validation Example)](./Plugin_System_Architecture/06_Data_Validation_Example.md)

### 5. 实现示例与指南 (Implementation Examples)
*动手写代码，从案例学习实践。*
...
* [OpenClaw UI 通道适配器 (OpenClaw UI Channel Adapters)](./Implementation_Examples/openclaw_UI_Channel_Adapters.md)
