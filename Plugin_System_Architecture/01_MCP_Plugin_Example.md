...
> 判定理由（掃描原文）：context.infrastructure.getService('message_broker') and the init/start(eventQueue) Listener from '@openstarry/core/interfaces' never existed (zero codebase hits); 'MCP' here means a broker-based agent bus, colliding with the real mcp-client/server (Model Context Protocol) plugins.
