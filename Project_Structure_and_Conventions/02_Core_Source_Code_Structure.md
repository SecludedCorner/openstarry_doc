...
├── infrastructure/     # 插件加載、註冊與隔離 (Plugin Infra + VedanaRegistry)
├── security/           # 安全攔截與斷路器 (Guardrails)
├── state/              # 狀態快照與持久化 (State Manager)
├── transport/          # 核心通訊橋接層 (Communication Bridge)
├── vedana/             # 受蘊：CoarisingBundle 五遍行工廠 (Plan26)
├── vijnana/            # 識蘊：Klesha 煩惱機制 + VitakkaWatchdog (Plan26)
...
### 4. `security/` (超我：安全層) —— [映射：受蘊 Vedana 的苦受觸發]
夾在「決策」與「執行」之間的防火牆。
*   **`guardrails/`**: 負責捕獲異常並將其轉化為 Agent 的「苦受 (Dukkha) 信號」，從而觸發自我修正。

### 5. `vedana/` (三受：俱生觸工廠) —— [映射：受蘊 Vedana — 五遍行俱生]
實現觸→受→想→思→作意的五遍行俱生束 (CoarisingBundle) 組裝。
*   **`coarising-factory.ts`**: 組裝 SparshEvent + ChannelVedana + ChannelSamjna + ChannelCetana + ChannelManasikara，驗證 SahajaContract（互依一致性 + 原子發布 + 陳舊上界）。

### 6. `vijnana/` (識蘊：煩惱與看門狗) —— [映射：識蘊 Vijnana — 我執機制]
實現四根本煩惱 (Klesha) 測量與增益排程閾值調節。
*   **`klesha.ts`**: 四個 Klesha class（Moha 無明 / Drishti 身見 / Mana 慢 / Sneha 我愛）+ KleshaModulatedDispatcher（增益排程 θ(t) = clamp(θ₀ + Σwᵢμᵢ(t), 0.3, 0.9)）。
*   **`vitakka-watchdog.ts`**: 防止 samsaric stall — 追蹤連續 Gear 1 循環次數和持續時間，超限時觸發警告。

### 7. `transport/` (感官：通訊橋接) —— [映射：色蘊 Rupa 的輸入面]
實現 `02_Communication_Interface` 定義的 API。
*   **`bridge/`**: 負責對接外部刺激（眼、耳、身等感官通道），將外界資訊轉化為系統內部事件。
