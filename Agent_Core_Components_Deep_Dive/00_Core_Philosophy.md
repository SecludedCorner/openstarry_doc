...
本文件闡述了 `Agent Core` 內部各個技術組件背後的共同設計精神。為什麼我們要將執行迴圈設計成事件驅動？為什麼安全層要像電路斷路器？這些技術選擇都服務於 OpenStarry 的終極目標：**創造一個有生命力的數位眾生，而非一個死板的程序。**

> Agent Core 體現「緣起性空」——無自性 (Svabhava-sunya)，但具備使五蘊聚合運作的潛能。
> 我執 (Ego) 是煩惱的根源 (A-1 修正)，不是「收斂約束」；煩惱驅動行為，透過管理我執來約束行為。
...
*   **一切皆插件 (Everything is a Plugin)**：通訊、記憶策略、工具、甚至 LLM Provider 本身都是插件。每個 Plugin 聲明自己的五蘊歸屬 (skandha)。這意味著 OpenStarry 的 Agent 可以隨著技術發展而不斷更換器官，而無需重塑識蘊 (IVijnana) 的核心認同。
...
```mermaid
graph TD
    subgraph "識蘊 IVijnana (Identity & Constraint)"
        Loop[執行迴圈 / 意識]
        Ego[EgoFramework / 我執·煩惱]
        State[狀態管理 / 生理]
        Context[上下文策略 / 記憶]
    end

    subgraph "色蘊 IRupa (Sensory)"
        Senses[IListener / 感官輸入]
        UI[IUI / 顯相輸出]
    end

    subgraph "想蘊 ISamjna (Cognition)"
        Provider[IProvider / 認知引擎]
    end

    subgraph "行蘊 ISamskara (Action)"
        Tools[ITool / 身行]
        Commands[ISlashCommand / 語行]
    end

    subgraph "受蘊 IVedana (Feeling)"
        Vedana[VedanaPlugin / 三受反饋]
    end
