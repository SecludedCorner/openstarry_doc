...
## Plugin 安全生命週期 (Plugin Security Lifecycle)

> [Cycle 02 Debate 5 整合] Plugin 安全生命週期管理。

### 框架概念

| 機制 | 功能 |
|------|------|
| Security Layer 策略引擎 | 預防——阻止未授權 Plugin |
| 安全更新機制 | 修復——撤銷已發現漏洞的 Plugin |
...
| 階段 | 元件 | 說明 |
|------|------|------|
| Plan24 | Plugin 黑名單 | `Set<pluginNameOrHash>` 永久封鎖 |
| Plan24 | CRL 檢查 stub | 憑證撤銷列表檢查 |
| Plan24 | Plugin 狀態欄位 | PluginManifest 新增 `status` |
| Plan27 | 完整狀態機 | 五種狀態之間的轉換 + 審計軌跡 |
| Plan27 | 隔離→啟用 | 人類核准工作流 |
| Plan-AC | 協調層戒律引擎 | 完整策略引擎 + 信任等級 |

### 三層安全模型 (與 Tenet #6 對應)

| 層級 | 機制 | 功能 | Tenet #6 對應 |
|------|------|------|--------------|
| Layer 0 (硬安全) | SafetyMonitor | 不可覆蓋的硬性限制 | — |
| Layer 1 (審議) | Klesha + IVolition | 煩惱驅動的行為評估 | 「kleshas and wisdom of vijnana」 |
| Layer 2 (觀察) | VedanaPlugin | 受蘊觀察，產生信號 | 「Feeling observes but does not intervene」 |

> [Cycle 02-5 D4-R3] 佛學裝飾標籤已清理。工程內容保留。
