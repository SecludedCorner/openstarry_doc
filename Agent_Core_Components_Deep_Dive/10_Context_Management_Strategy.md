...
## Plan32 Wave 6: Context Manager 提取為 Plugin

`[Cycle 02-8 新增]`

### 背景

在 Plan32 之前，context manager 的 `createContextManager()` 工廠函數硬編碼在 Core 中，實作滑動窗口策略。這違反了 Tenet #9（可插拔的記憶策略）——雖然 IContextManager 介面存在，但實際上無法透過 PluginLoader 替換策略。

### Plan32 Wave 6 設計 (02-8_c 修正版)

Plan32 Wave 6 將 context manager 從 Core 提取為獨立 plugin：

| 項目 | 修正前 | 修正後 |
|------|--------|--------|
| 實作位置 | `packages/core/src/context/` | `@openstarry-plugin/context-sliding-window` |
| Core 行為 | `createContextManager()` 硬編碼 | 透過 PluginLoader 載入 |
| 缺席行為 | 不可能缺席（硬編碼） | `throw Error('Context manager plugin required')` |
| 關鍵性級別 | — | **Required** |

### 三級關鍵性模型

Context manager 被分類為 **Required** 級別——這與 Auditor (Optional-degraded) 不同：

- **Context manager 缺席**: Agent 無法組裝 LLM payload → 無法運行 → throw Error
- **Auditor 缺席**: 路由仍可正常進行，delta=0 → 安全增強層未啟用，其餘功能正常

### 設計一致性

02-8_b 原始設計提出 `pluginContextManager ?? createContextManager()` fallback，但這直接矛盾 Wave 1 移除 `?? createThresholdAuditor()` 的原則。02-8_c 修正為 `throw Error()` 行為，與微核心純淨原則一致。

### 開發者影響

Agent 配置中需要明確指定 context strategy plugin：

```jsonc
{
  "plugins": [
    { "name": "@openstarry-plugin/context-sliding-window" },
    // 或者使用第三方 context strategy
    { "name": "@my-org/context-dynamic-summarization" }
  ]
}
```

Starter template 預先包含 `@openstarry-plugin/context-sliding-window`，確保新 Agent 開箱即用。