---
title: Reference/20 — Audit Verdict 格式 Codification（中文版）
...
supersedes: (無 — first canonical codification)
cross_refs:
...
# Reference/20 — Audit Verdict 格式 Codification（中文版）

**Status**：BINDING — cycle 03-25 R3 §1.2 D-§13 super-majority 22/1（LEIBNIZ DSS-CY25-§13-A 永久保留 per MR-11）。Pending Master Ratification Batch 22 #7。

---

## §1 背景

Cycle 03-26 繼承 **530-680 cell 矩陣 sweep**（cell = `<chapter_id>_<tenet_axis>`；D-§12 cycle 03-25 governs enumeration）。每 cell 攜帶 audit verdict；aggregated row + column 結果 feed cycle 03-26 GP-coherence pre-gate 和 downstream G-gates。

Pre-cycle-03-25 verdicts 為 ad-hoc strings（`PASS`、`PASS UNCONDITIONAL`、`CONDITIONAL PASS`、`FAIL`、bare prose）。兩個 operational hazard：(a) machine-parseability 在 ~5,300-6,800 verdict tokens 阻塞（3-4 person-weeks 手工；6-8 hours linted）；(b) verdict 語義 ambiguity（cycle 03-21 R3 §6 audit_calc state-(a) 顯示 3 個 false-positive cells 有 unresolved cross-axis dependencies）。

**Prerequisite**：cycle 03-26 矩陣 sweep cell verdict 依本格式 binding。

---

## §2 4-tier verdict 分類 + N/A 5th lane

依強度遞減：

1. **`full_pass`** — 全 Tenet sub-checks GREEN；無 caveats；無 roll-forward。
2. **`conditional_pass`** — GREEN under explicit named caveats（`caveats:` field non-empty）。**audit-context only** 允許 post-MR-7；**G-gate context 禁用**。N=3 consecutive cycles roll-forward ceiling（R2-D §5）。
3. **`fail`** — 一個或多個 sub-checks RED；下 cycle 全 re-audit。
4. **`fail_fix=<scope>`** — RED 但 bounded fix scope（free-form ASCII identifier 1-128 chars）。下 cycle re-audit on `<scope>` 限定範圍。

**N/A 5th lane**：`not_applicable` — cell × Tenet axis combination 不適用。**非 verdict**；lane-distinguished from 4-tier ladder。Accounted in per-row `applicable_count` 分母 only。

強度順序：`full_pass > conditional_pass > {fail, fail_fix=<scope>}`。兩種 FAIL 形式 not strictly ordered（`fail_fix=*` operationally softer；semantically identical-or-weaker）。
...
| ID | Rule |
|----|------|
| L1 | `verdict` token 在 5-token canonical enum |
...
**R2-E SHALL splits per method tier**：每 evidence channel SHALL 標 either `SHALL_AUTOMATIC`（lint-enforced）or `SHALL_REVIEWED`（human-gate via `reviewer.secondary`）。
...
**Mixed PASS** 為本 codification novel：區分「row 多數 green 但有 1-2 caveat cells」與 blanket conditional_pass。Audit-context 接受；flagged for cycle-N+1 caveat closure。
...
**Determinism**：deterministic by `cell_id ASC` tiebreak；兩 runs over same input MUST 產生 byte-identical aggregate（cycle 03-26 R3 sanity check）。
...
per I-13.7 + R2-B lint enforcement，**4 個 citation channels**（no free-form）：
...
**R2-E method tier split**：each evidence channel SHALL be `SHALL_AUTOMATIC`（lint validates: file_paths exist, test_ids resolve）or `SHALL_REVIEWED`（reviewer.secondary attests）。兩 tiers may co-exist on one cell。
...
**Verbatim text**（preserved per MR-11；EN 原版見英文檔案；本中文版保留 verbatim per MR-11 EN-only 原則）：
...
Cycle 03-26 R0 orientation 繼承 Reference/20 為 canonical verdict format。CI hook `tools/audit_verdict_lint.py`（ARCHIMEDES O3 plan-spec implementation slot，~150-220 Python LOC）deploy at cycle 03-26 R1 close + R2 close + R3 pre-gate。Non-conforming cells fail G4-folder-3 BINDING canonical doc creation gate。

Mixed PASS row class（§4.1）為本 codification novel；expected ~15-25% of total rows in cycle 03-26 sweep。Tenet #10 column expected ~30-50% `full_pass_pct` until Phase 6 closure（per MR-5 NC PENDING hard cap）。

---

**END Reference/20（中文版）。**
