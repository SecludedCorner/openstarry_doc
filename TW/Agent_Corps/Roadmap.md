...
| **v0.20.1-beta** | **Cycle 25 hotfix** | **Windows 跨平台修復 (43→0 failures)** | **2026-02-13** | **✅ COMPLETE** |
| v0.21.0-beta | Cycle 26 | Plan23 (TBD — 待研究團隊建議確認) | TBD | 📋 PLANNED |
...
- IGuide (識) — System prompts/guides
...
- Windows 跨平台修復：43 個失敗測試 → 0 個失敗
- 路徑處理：`sep`、`basename()`、`pathToFileURL()` 取代硬編碼 Unix 路徑
- Daemon IPC：新增 `platform.ts` — Windows 用 named pipe，Linux 用 Unix socket
- 平台 guard：SIGHUP、chmod、mkdirSync、unlinkSync 在 Windows 上跳過
- `plugin-installer.ts`：`cp()` dereference fallback + `rm()` maxRetries
- 測試數量：1330 → 1332（+2，3 skipped）
...
**Next Cycle**: Plan23 — 待研究團隊建議確認後分配內容

**Backlog 候選** (未排序):
- State Machines & Event Routing (已完成探索，規格草案就緒)
- Multi-agent Orchestration (需 2-3 cycles)
- Service Mesh Patterns
- Plugin Signature Verification Integration (plugin-signer 已存在但未啟用)
- Network Fetch Sandbox (allowedDomains 已定義但未實作)

**Status**: Plan01-22 全部完成 ✅ | 測試: 1330 | snapshot: 20260213_cycle25
