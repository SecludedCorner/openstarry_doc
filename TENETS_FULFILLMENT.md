...
> 這份文件是寫給未來的。
>
> 十大宣言（canonical：[README §十大核心宣言](./README.md#-十大核心宣言-the-ten-tenets)）是這個專案對「AI 落地那一天」的預先回答。本帳本逐條記錄每一條宣言在 v0.59.x-alpha（最終驗證 v0.59.4）的**誠實兌現狀態**：哪些已被可運行的 code＋通過的測試證明、哪些只證明到了明確標注的範圍、哪些仍是留給後人的未竟工程。
>
...
| 9 | 可插拔的記憶策略 | ✅ **已證明** | context-sliding-window／context-summary 可換裝；IContextManager 為強制插件類別。workflow 執行狀態自 v0.58 可落盤（`OPENSTARRY_WORKFLOW_STATE_DIR`）。**CLI 模式對話歷史自 v0.59.4-alpha 亦可落盤**：關機時存檔、`--resume` 還原，沿用 daemon 的 `FileSessionPersistence` store（`apps/runner/src/utils/cli-session-persistence.ts` 接通 `start.ts`，`cli-session-persistence.test.ts` 4 測試）。 |
| 10 | 分形社會結構 | 🟢 **depth=3 已證明；機制同構 ⇒ 任意有限深度無已知障礙（受資源上界約束）** | v0.59 端到端實證 depth=2；**v0.59.1 升至 depth=3**：一個外部呼叫穿越三代進程（父→中→孫，各自完整認知迴圈），逐層蓋章返回 `PARENT-FINAL:MID-FINAL:CHILD-ANSWER:<孫代PID>`（`fractal-depth3.e2e.test.ts`，<2s）——**每一層用完全相同的機制（agent-ask＋mcp-server＋mcp-client），遞迴結構本身就是「由一而生萬物」**（語出道家「道生一⋯三生萬物」之轉用，非佛教語彙——中觀正面批判一因生萬法，此處僅取分形意象）。承重件＝新插件 `agent-ask`（把認知迴圈暴露為可委派工具；此前 MCP 組合的只是工具表）。v0.59.1 同時修真 daemon 層：root 自註冊（processTree 出廠即空殼→真樹）、spawnChild 樹邊＋SEC-003 拒絕實測、**父亡子收**（孤兒回收級聯首次存在，`daemon-process-tree.e2e.test.ts`）。**運行中自主生子（v0.59.4-alpha，2026-06-15）**：新插件 `agent-spawn` 把生子暴露為 `agent.spawnChild` ITool，agent 自己的認知迴圈可呼叫；daemon 註冊 `SERVICE_KEYS.DAEMON_SPAWN` service（背後走 handleSpawnChild，沿用 A② 的權限格＋SEC-003），非 daemon 色身回明確「daemon-only」訊息（`agent-spawn/src/index.test.ts` 3 測試：生子成功／權限拒絕／無 daemon）。**明確不宣稱**：路由是 MCP（MessageRouter／channel／comm-pipeline 為驗證層或未接線）；depth>3 未實測（但每層機制同構，歸納論證成立）；LLM-initiated 生子的端到端真進程鏈未另立 e2e（工具→service 由單測覆蓋、service→真進程由 daemon-process-tree e2e 覆蓋）。 |
