...
1. **New Plugin**: `@openstarry-plugin/tui-dashboard` with IUI (Rupa) + IListener (Vedana) interfaces
...
/**
 * TuiListener implements the IListener interface (Vedana / Feeling Aggregate).
...
/**
 * TuiUI implements the IUI interface (Rupa / Form Aggregate).
 ...
 *
 * Provides:
 * - TuiUI (Rupa / Form Aggregate) — renders agent events in terminal
 * - TuiListener (Vedana / Feeling Aggregate) — receives user input
 *
...
- `/data/openstarry_eco/share/openstarry_doc/Plugin_System_Architecture/00_Plugin_Philosophy_Five_Aggregates.md`
  - IUI (Rupa / Form Aggregate) interface
  - IListener (Vedana / Feeling Aggregate) interface
