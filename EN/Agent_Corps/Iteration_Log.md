...
- **Scope**:
  - MCP Server Mode: OpenStarry acts as an MCP server, exposing tools/resources/prompts to external clients
  - MCP Client Mode: OpenStarry connects to external MCP servers, importing external tools
  - MCP three core capabilities: tool, resource, prompt
  - Transport layer support: stdio, HTTP+SSE
- **Pre-Conditions**:
  - Plan05.1 Session Isolation ✅ (v0.2.1-beta, Cycle 1)
  - Cycle 2 ✅ (v0.2.2-beta, 165 tests)
  - Plan06 Entry Checklist: Session isolation ✅, Multi-client scenarios ✅, No cross-session message leakage ✅
- **Strategy**: Standard SOP (Phase 0 → 1 → 1.5 → 2 → 2.5 → 3 → 4)

### Phase 0 — Planning

- **researcher**: Pre-researched MCP Protocol technical approach (official spec, openoctopus reference, Five Aggregates mapping)
  - Report: `share/test/reports/research/20260211_cycle3/Research_MCP_Protocol.md`
- **doc-keeper**: Documented iteration plan and scope decisions (this record)
- **Scope Decisions** (based on researcher recommendations):
  1. **MCP Client Plugin only** — `@openstarry-plugin/mcp-client`; MCP Server Plugin deferred to a future Cycle
  2. **MCP primitives**: Tool bridge + Prompt bridge (Phase 1); Resources deferred
  3. **Transport**: stdio + Streamable HTTP (custom JSON-RPC, no dependency on @modelcontextprotocol/sdk)
  4. **Protocol version**: 2024-11-05 (proven, compatible; upgradeable to 2025-11-25 in future)
  5. **Five Aggregates mapping**: MCP Tools → ITool (Samskara), MCP Prompts → SlashCommand (user-controlled), MCP Resources → defer (no new Aggregate added)
  6. **Tool naming**: `serverName/toolName` prefix to avoid conflicts
...
- **Scope**:
  - MCP Server Plugin: OpenStarry acts as an MCP server, exposing tools and prompts to external clients
...
- **Pre-Conditions**:
  ...
  - Worker process hardening is next priority (prevent hangs, enable subscriptions, graceful restarts)
...
- **Pre-Conditions**:
  ...
  - Advanced hardening is next priority (static analysis, pool optimization, cryptographic signing)
...
- **Scope**:
  ...
  - IListener (Vedana Aggregate) implementation for TUI keyboard input → ctx.pushInput()
...
- **Pre-Conditions**:
  ...
  - Five Aggregates architecture: IListener (Vedana Aggregate) + IUI (Rupa Aggregate) coexist in same plugin
- **Strategy**:
...
- **Key Decisions Recorded**:
  ...
  2. **IListener coexistence**: IUI (Rupa Aggregate) + IListener (Vedana Aggregate) live in same plugin factory
...
- **Roadmap Impact**:
  ...
  - Five Aggregates now fully operational: IUI (Rupa) + IListener (Vedana) in same plugin ecosystem
...
- Five Aggregates: ITool (Samskara Aggregate) primary + SlashCommand + IPluginService
