...
| **v0.20.1-beta** | **Cycle 25 hotfix** | **Windows Cross-Platform Fix (43→0 failures)** | **2026-02-13** | **✅ COMPLETE** |
| v0.21.0-beta | Cycle 26 | Plan23 (TBD — pending research team recommendations) | TBD | 📋 PLANNED |
...
### Phase 5: Next Generation (PLANNED 📋)
  - **Plan23**: TBD — pending assignment after research team recommendations

  - **Backlog** (unordered, pending priority evaluation):
...
- IUI (Form) — User Interface handlers
- IListener (Sensation) — Event listeners
- IProvider (Perception) — Service providers
- ITool (Volition) — Executable tools
- IGuide (Consciousness) — System prompts/guides
...
- Plan23: TBD — pending research team recommendations
...
Plan22 (Plugin Marketplace MVP) ✅
  ↓
Plan23 (TBD — pending research team recommendations) 📋 PLANNED
  ↓
Backlog: State Machines, Multi-agent, Service Mesh, Plugin Signature, Network Sandbox
```
...
- Windows cross-platform fix: 43 failing tests → 0 failures
- Path handling: replaced hardcoded Unix paths with `sep`, `basename()`, `pathToFileURL()`
- Daemon IPC: added `platform.ts` — named pipe on Windows, Unix socket on Linux
- Platform guards: SIGHUP, chmod, mkdirSync, unlinkSync skipped on Windows
- `plugin-installer.ts`: `cp()` dereference fallback + `rm()` maxRetries
- Test count: 1330 → 1332 (+2, 3 skipped)
...
**Next Cycle**: Plan23 — content to be assigned after research team recommendations are confirmed

**Backlog Candidates** (unordered):
- State Machines & Event Routing (exploration complete, spec draft ready)
- Multi-agent Orchestration (estimated 2-3 cycles)
- Service Mesh Patterns
- Plugin Signature Verification Integration (plugin-signer exists but not yet enabled)
- Network Fetch Sandbox (allowedDomains defined but not yet implemented)

**Status**: Plan01-22 all complete ✅ | Tests: 1330 | Snapshot: 20260213_cycle25
