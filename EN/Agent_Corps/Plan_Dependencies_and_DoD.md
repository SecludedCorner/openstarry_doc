...
Plan05 (Multi-Channel UI & Listener)    ✅ v0.2-beta   2026-02-07
  ...
  ├── Plan05.3 (DevTools)              ⬜ → Deferred until after Plan06
  ├── Plan05.4 (E2E Testing)           ⬜ → To be scheduled
  │     ↓
  ...
  │
  └── (Extended Mode: New plugins, Web UI, Security audit)
```
...
Plan06 (MCP Client + Server) is complete ✅. **Plan07 (Runtime Sandbox) has been unblocked** and is the next item to implement on the critical path.

### Parallelizable Work

| Parallelizable | Description |
|----------------|-------------|
| Plan06 implementation + Plan05.3/05.4 | DevTools / E2E testing can run in parallel with MCP |
| Plan06 pre-research + Cycle 2 wrap-up | ✅ Complete — researcher can pre-research Plan06 |
...
Each Plan must satisfy **all of the following conditions** before it can be marked ✅:

| # | Condition | Verification Method |
|---|-----------|---------------------|
| 1 | `pnpm build` passes (monorepo + plugins) | qa validates in agent_test |
| 2 | `pnpm test` all pass, and test count ≥ previous cycle baseline | Regression check in qa report |
| 3 | `pnpm test:purity` passes | Purity check in qa report |
| 4 | All items in Architecture_Spec are implemented | architect Code Review PASS |
| 5 | Five Aggregates compliance (new components correctly categorized) | architect Code Review confirmation |
| 6 | pushInput pattern compliance | architect Code Review confirmation |
| 7 | No known security vulnerabilities | architect Code Review confirmation |
| 8 | Dev log has been written | Coordinator confirms file exists |
| 9 | QA Report and Code Review are both PASS | Phase 4 determination |
| 10 | doc-keeper has updated Plan with ✅ + Iteration_Log | Phase 4 convergence |
| 11 | Snapshot has been created | `scripts/snapshot.sh` completed |
| 12 | Lessons Learned have been recorded | Phase 4 retrospective |

### 2.2 Plan-Specific Acceptance Criteria

#### Plan05.1: Session Isolation ✅ (Cycle 1, 2026-02-10)
- [x] WebSocket connection supports session token / auth (session handshake on connect)
- [x] Multiple client connections are isolated from each other (no session data cross-contamination)
- [x] A single agent instance can serve multiple sessions (SessionManager + default session)
- [x] Session lifecycle management (create, maintain, destroy)
- [x] Added test coverage for session isolation scenarios (17 tests)

#### Plan05.2: HTTP SSE ✅ (Cycle 1, 2026-02-10)
- [x] HTTP Server-Sent Events transport plugin available (GET /api/events)
- [x] SSE connection can receive real-time streaming responses
- [x] Coexists with existing HTTP webhook listener without conflicts
- [x] Added test coverage for SSE scenarios (11 tests)

#### Plan05.5-①: Health Check ✅ (Cycle 1, 2026-02-10)
- [x] WebSocket protocol ping/pong + stale connection cleanup
- [x] HTTP SSE heartbeat
- [x] Added test coverage (6 tests)
...
- [x] Transport plugin structured logging migration
- [x] Added test coverage (19 tests)
...
- [x] Added test coverage (16 tests)

#### Plan06: MCP Protocol (Model Context Protocol) — Phased

**Cycle 3 (Plan06 Phase 1: MCP Client)** ✅ (Cycle 3, 2026-02-11):
- [x] `@openstarry-plugin/mcp-client` plugin available
- [x] MCP client mode: connect to external MCP server, import tools (Tool bridge: MCP tools → ITool)
- [x] MCP Prompt bridge: import MCP prompts as SlashCommand
...
- [x] Added test coverage for MCP client scenarios (35 tests)

**Cycle 4 (Plan06 Phase 2: MCP Server)** ✅ (Cycle 4, 2026-02-11):
- [x] `@openstarry-plugin/mcp-server` plugin
- [x] MCP server mode: expose ITool/IGuide as MCP tools/prompts
...
- [x] Added test coverage for MCP server scenarios (52 tests)
...
- [x] Added test coverage for MCP resources + auth scenarios (29 tests)
...
- [x] Added test coverage for sampling/logging/roots/bidirectional scenarios (87 tests)
...
- [x] Plugin execution environment isolation (Node.js vm.createContext + worker_threads)
- [x] Resource limits (CPU watchdog deadline timer, memory sandbox isolation, file access)
- [x] Plugin signature verification mechanism (SHA-512 hash + Ed25519 PKI support)
- [x] Added test coverage for sandbox escape scenarios (68 tests)
- Sandbox version: v0.4 (Cycle 5)
...
- [x] Added test coverage (31 tests)
- Sandbox version: v0.4.1-beta (Cycle 6)
...
- [x] Added test coverage (55 tests)
- Sandbox version: v0.4.2-beta (Cycle 7)
...
- [x] Added test coverage (53 tests)
- **Version**: v0.5.0-beta
...
- [x] IListener (Vedana / Feeling Aggregate) implementation for keyboard input
...
- [x] Added test coverage for interactive input scenarios (35 tests)
...
- [x] Added test coverage (44 tests)
...
- [x] Added test coverage (33 tests)
...
- [x] Added test coverage (15 tests: session persistence, multi-client, tab-completion, backpressure)
...
- [x] Added test coverage (39 tests: 19 scanner + 10 sync + 5 E2E + 5 resolver)
...
## 3. Iteration Schedule Recommendations

| Iteration | Plans Covered | Target Version | Status |
|-----------|---------------|----------------|--------|
| Cycle 1 | Plan05.1 + 05.2 + 05.5-① | v0.2.1-beta | ✅ Complete (2026-02-10, 118 tests) |
| Cycle 2 | Plan05.5-② + 05.5-③ | v0.2.2-beta | ✅ Complete (2026-02-11, 165 tests) |
| Cycle 3 | Plan06 Phase 1 (MCP Client) | v0.3.0-beta | ✅ Complete (2026-02-11, 200 tests) |
| Cycle 4 | Plan06 Phase 2 (MCP Server) | v0.3.1-beta | ✅ Complete (2026-02-11, 252 tests) |
| Cycle 5 | Plan07 (Sandbox MVP) | v0.4 | ✅ Complete (2026-02-11, 320 tests) |
| Cycle 6 | Plan07.1 (Sandbox Hardening) | v0.4.1-beta | ✅ Complete (2026-02-11, 351 tests) |
| Cycle 7 | Plan07.2 (Advanced Hardening) | v0.4.2-beta | ✅ Complete (2026-02-11, 407 tests, 1 rework) |
| Cycle 8 | Plan07.3 (Custom require + audit) | v0.4.3-beta | ✅ Complete (2026-02-11, 442 tests) |
| Cycle 9 | Plan08 (TUI Dashboard MVP) | v0.5.0-beta | ✅ Complete (2026-02-11, 524 tests) |
| Cycle 10 | Plan09 (Interactive TUI) | v0.5.1-beta | ✅ Complete (2026-02-12, 559 tests) |
| Cycle 11 | Plan10 (CLI Foundation & Runner) | v0.6.0-beta | ✅ Complete (2026-02-12, 632 tests, 1 rework) |
| Cycle 12 | Plan11 (DevTools & E2E Testing) | v0.7.0-beta | ✅ Complete (2026-02-12, 670 tests, 1 rework) |
| Cycle 13 | Plan12 (Daemon Mode MVP) | v0.8.0-beta | ✅ Complete (2026-02-12, 714 tests, 1 rework) |
| Cycle 14 | Plan13 (Seamless Attach) | v0.9.0-beta | ✅ Complete (2026-02-12, 747 tests, 1 rework) |
| Cycle 15 | Plan06-P3 (MCP Resources + OAuth) | v0.10.0-beta | ✅ Complete (2026-02-12, 792 tests) |
| Cycle 16 | Plan14 (Multi-client Attach & Session Management) | v0.11.0-beta | ✅ Complete (2026-02-12, 807 tests) |
| Cycle 17 | Plan06-P4 (MCP Sampling & Advanced Protocol Extensions) | v0.12.0-beta | ✅ Complete (2026-02-12, 894 tests) |
| Cycle 18 | Plan15 (SDK Context Extensions & Provider Integration) | v0.13.0-beta | ✅ Complete (2026-02-12, 915 tests) |
| Cycle 19 | Plan16 (Security Hardening & Quality Polish) | v0.14.0-beta | ✅ Complete (2026-02-12, 935 tests, 1 rework) |
| Cycle 20 | Plan17 (Plugin Developer Experience) | v0.15.0-beta | ✅ Complete (2026-02-12, 970 tests) |
| Cycle 21 | Plan18 (Plugin Sync & System Plugin Directory) | v0.16.0-beta | ✅ Complete (2026-02-12, 1009 tests) |
| Cycle 22 | Plan19 (Plugin Dependency Wiring & Cross-Plugin Services) | v0.17.0-beta | ✅ Complete (2026-02-12, 1067 tests, 1 rework) |
| Cycle 23 | Plan20 (Workflow Engine MVP) | v0.18.0-beta | ✅ Complete (2026-02-12, 1104 tests, spec rewrite) |
| Cycle 24 | Plan21 (Web-based Remote Attach) | v0.19.0-beta | ✅ Complete (2026-02-12, 1132 tests, +28 net) |
| Cycle 25+ | Plan22+ (Advanced Features: Plugin Marketplace, Multi-agent Orchestration) | v0.20.0+ | ⬜ To be scheduled |

---

## 4. Usage

- **Coordinator** should consult this document before starting a new iteration to confirm:
  1. Whether the prerequisite Plan(s) are complete ✅
  2. The plan-specific acceptance criteria
  3. Whether there is parallelizable work
- **architect** should reference the plan-specific acceptance criteria when designing the Spec to ensure coverage
- **qa** should check both general DoD and plan-specific conditions item by item during verification
- **doc-keeper** should confirm all DoD conditions are met when marking a Plan ✅
